...
**[Try the live demo →](https://rupeeguard-fx4tblwtvzxbcbvhy6fef2.streamlit.app/)**
...
**[rupeeguard-fx4tblwtvzxbcbvhy6fef2.streamlit.app](https://rupeeguard-fx4tblwtvzxbcbvhy6fef2.streamlit.app/)**

Enter a failed transaction's details — amount, method, attempts, device-sharing count, and failure reason — and get an instant decision from the actual trained model: blocked as fraud, recovered, or escalated to manual review, along with the reasoning behind it.

---

## Sample results

The chart below is a static snapshot from evaluating the pipeline on a held-out test set of 600 failed transactions (₹11.65L total value) that the fraud model had not seen during training. It is not live in the deployed app — the app above focuses on single-transaction testing; this shows the full-batch picture.
...
**Try it instantly:** [rupeeguard-fx4tblwtvzxbcbvhy6fef2.streamlit.app](https://rupeeguard-fx4tblwtvzxbcbvhy6fef2.streamlit.app/) — no setup required.

**Run it yourself:**
1. Open `notebooks/RupeeGuard.ipynb` in [Google Colab](https://colab.research.google.com/) or a local Jupyter environment. The notebook generates its own synthetic dataset, so no external data is required.
2. To run the app locally: `pip install -r requirements.txt` then `streamlit run streamlit_app.py`.
3. The optional LLM recovery agent step (in the notebook) requires a free Gemini API key from [Google AI Studio](https://aistudio.google.com) — no billing setup needed on the free tier.

## Tech stack

Python, Pandas, scikit-learn, XGBoost, SHAP, Google Gemini API, Streamlit.
...
- The LLM-based recovery agent is demonstrated on a sample of transactions rather than the full batch, due to free-tier API rate limits. The rule-based path is used for full-batch throughput and powers the live demo, with the LLM agent available as a higher-reasoning option for ambiguous cases.
